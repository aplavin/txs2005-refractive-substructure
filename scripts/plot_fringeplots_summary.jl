...
let
	...
	@p all_fringefits filter(any(r -> r.band isa AbstractVector, _.fringefits)) flatmap() do r
		...
		Label(col[0,1:2], rich(f"""VLBA observations: {freqstr}, in {datestr}"""); tellwidth=false)
		...
		for (i, r) in enumerate(@p freqgr sort(by=_.date_obs) first(__, 20))
			fplt = FPlot(
				...
				VLBIPlots.F.UVdist(),
				AxFunc(limit=(-0.01, nothing), scale=SymLog(5; linscale=0.2), label="Fringe SNR", @o _.peakval/_.noisedist.σ);
				color, markersize=4,
			)
